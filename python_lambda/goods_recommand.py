...
OPENSEARCH_USER = "store"
OPENSEARCH_PASSWORD = "store1!"
...
def analyze_text_list(text_list: list[str]) -> list[str]:
    res = requests.post(f"{OPENSEARCH_HOST}/store/_analyze", auth=auth, verify=False,
                        headers={
...
def search(keywords: list[str]) -> list[int]:
    ...
    res = requests.post(f"{OPENSEARCH_HOST}/store/_search", auth=auth, verify=False,
                        headers={
                            'Content-Type': 'application/json'
                        },
                        json=query)
