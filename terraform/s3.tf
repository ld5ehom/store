...
module "s3_list" {
  for_each = toset([
    ...
    "store-profile-${local.uid}",
    "store-goods-${local.uid}",
...
data "aws_iam_policy_document" "content_s3_policy" {
  statement {
    actions = ["s3:GetObject"]
    resources = [
      "${module.s3_list["store-goods-${local.uid}"].s3_bucket_arn}/*"
    ]
...
resource "aws_s3_bucket_policy" "content" {
  bucket = module.s3_list["store-goods-${local.uid}"].s3_bucket_id
