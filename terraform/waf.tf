resource "aws_wafv2_ip_set" "frontend" {
  ...
  description = "IP set for frontend access"
  scope       = "CLOUDFRONT"
...
resource "aws_wafv2_web_acl_association" "backend_stage" {
  ...
  resource_arn = "arn:aws:apigateway:us-west-1::/restapis/${local.api_gateway_id}/stages/Stage"
  web_acl_arn  = aws_wafv2_web_acl.backend.arn
}

resource "aws_wafv2_web_acl_association" "backend_prod" {
  count = local.api_gateway_id == "" ? 0 : 1
  resource_arn = "arn:aws:apigateway:us-west-1::/restapis/${local.api_gateway_id}/stages/prod"
  web_acl_arn  = aws_wafv2_web_acl.backend.arn
}
