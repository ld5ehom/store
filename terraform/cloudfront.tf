...
resource "aws_cloudfront_origin_access_identity" "s3_origin_access_identity" {
  comment = "store origin_access_identity for s3"
}
...
resource "aws_cloudfront_distribution" "content" {
  ...
  origin {
    connection_attempts = 3
    connection_timeout  = 10
    domain_name         = module.s3_list["store-goods-${local.uid}"].s3_bucket_bucket_regional_domain_name
